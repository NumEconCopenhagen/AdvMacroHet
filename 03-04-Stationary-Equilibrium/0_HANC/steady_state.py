...
1. find_ss (general function wrapper)
2. find_ss_direct (direct method)
3. find_ss_indirect (indirect method)
4. find_ss_beta (beta method)
"""
    
def find_ss(model,method='direct',do_print=False,beta_min = 0.95, beta_max = 0.98,K_min=1.0,K_max=10.0,NK=10):
    ...
    if method == 'direct':
        ...
        find_ss_beta(model,do_print=do_print,beta_min = beta_min, beta_max = beta_max)
    else:
...
def find_ss_direct(model,do_print=False,K_min=0.1,K_max=10.0,NK=10):
    ...
    root_finding.brentq(
        obj_ss,K_min,K_max,args=(model,'direct'),
        do_print=do_print,varname='K_ss',funcname='A-A_hh'
    )


def find_ss_beta(model, beta_min=0.93, beta_max=0.95, do_print=True):
    par = model.par
    ss = model.ss
    ...
    par.delta = ss.rK - par.r_ss_target
    par.Gamma_ss = ss.Y/(ss.K**par.alpha)
    ss.Gamma = par.Gamma_ss
    ss.w = (1-par.alpha) * ss.Y

    root_finding.brentq(
        obj_ss,beta_min,beta_max,args=(model, 'beta'),
        do_print=do_print, varname='beta',funcname='A-A_hh'
    )
