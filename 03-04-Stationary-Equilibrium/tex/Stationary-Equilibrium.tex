...
\begin{document}
\title{3-4. Stationary Equilibrium\vspace{-2mm}}
...
\begin{itemize}
\item <+->\textbf{Production function: $Y_{t}=F(\Gamma_{t},K_{t-1})$ }{\footnotesize{[}capital
...
\begin{align*}
u^{\prime}(C^{hh}_{t}) & =\beta(1+F_{K}(\Gamma_{t},K_{t},1)-\delta)u^{\prime}(C^{hh}_{t+1})\\
...
\end{align*}
\item Important thing to note: the steady-state asset supply is completely
inelastic!
\end{itemize}
\end{frame}
%
...
\begin{align*}
v_{0}(z_{it},a_{it-1}) & =\max_{\{c_{it}\}^{\infty}_{t=0}}\mathbb{E}_{0}\sum^{\infty}_{t=0}\beta^{t}u(c_{it})\\
 & \text{s.t.}\\
\ell_{it} & =z_{it}\\
...
\begin{align*}
v_{}(z_{it},a_{it-1}) & =\max_{c_{t}}u(c_{t})+\beta\mathbb{E}\left[v(z_{it+1},a_{it})\right]\\
 & \text{s.t.}\\
\ell_{it} & =z_{it}\\
a_{it} & =(1+r_{t})a_{it-1}+w_{t}\ell_{it}-c_{it}\\
...
\item Aggregates $\left(w_{t},r_{t}\right)$
\end{itemize}
\item <+-> We can use those to compute the distribution of households $\boldsymbol{D_{t}}(z_{it},a_{it-1},\{r_{\tau},w_{\tau}\}_{\tau\geq t})$
\item <+-> Aggregate policy are obtained combining policies and distributions:
\[
X^{hh}_{t}\left(\left\{ r_{\tau},w_{\tau}\right\} _{\tau\geq t}\right)=\int x^{\ast}_{t}(z_{it},a_{it-1},\left\{ r_{\tau},w_{\tau}\right\} _{\tau\geq t})d\boldsymbol{D}_{t}
\]
\item <+->When aggregating we \textbf{integrate} out individual states 
\begin{itemize}
\item Aggregate $X^{hh}_{t}$ is only a function of $\left\{ r_{\tau},w_{\tau}\right\} _{\tau\geq t}$
in GE as long as exogenous states don't change 
\end{itemize}
\item <+->$\Rightarrow$ If we know aggregates $\left(w_{t},r_{t}\right)$
...
\item <+->HANC with $N_{z}=7,N_{a}=300:$ $8+7\times300=2108$ per period
\end{itemize}
\end{itemize}
\end{frame}
%
...
\end{frame}
%
\begin{frame}{Direct implementation (calibration)}
...
\[
c^{-\sigma}_{it}=\beta_{i}(1+r_{t+1})\mathbb{E}_{t}\left[c^{-\sigma}_{it+1}\right]
\]
}
...
\text{s.t. }a_{it}+c_{it} & =(1+r_{t})a_{it-1}+w_{t}z_{it}\geq0\\
\log z_{it+1} & =\rho_{z}\log z_{it}+\psi_{it+1}\,\,\,,\psi_{it}\sim\mathcal{N}(\mu_{\psi},\sigma_{\psi}),\,\mathbb{E}[z_{it}]=1\,
\end{align*}
...
\text{s.t. }p^{B}_{t}a_{it}+c_{it} & =a_{it-1}+(1-\tau_{t})z_{it}\geq0\\
\log z_{it+1} & =\rho_{z}\log z_{it}+\psi_{it+1}\,\,\,,\psi_{it}\sim\mathcal{N}(\mu_{\psi},\sigma_{\psi}),\,\mathbb{E}[z_{it}]=1\,
\end{align*}
