...
$Y_{t}=w_{t}L_{t}+r_{t}^{K}K_{t-1}$ {\footnotesize{[}functional income
...
\begin{align*}
v_{0}(z_{it},a_{it-1}) & =\max_{\{c_{it}\}_{t=0}^{\infty}}\mathbb{E}_{0}\sum_{t=0}^{\infty}\beta^{t}u(c_{it})\\
 & \text{s.t.}\\
\ell_{it} & =z_{it}\\
...
\[
X_{t}^{hh}\left(\left\{ r_{\tau},w_{\tau}\right\} _{\tau\geq t}\right)=\int x_{t}^{\ast}(z_{it},a_{it-1},\left\{ r_{\tau},w_{\tau}\right\} _{\tau\geq t})d\boldsymbol{D}_{t}
\]
\item <+->When aggregating we \textbf{integrate} out individual states 
\begin{itemize}
\item Aggregate $X_{t}^{hh}$ is only a function of $\left\{ r_{\tau},w_{\tau}\right\} _{\tau\geq t}$
...
\begin{itemize}
\item \textbf{Capital market:} $K_{t}=A_{t}=\int a_{t}^{\ast}(z_{it},a_{it-1})d\boldsymbol{D}_{t}$
\item \textbf{Labor market:} $L_{t}=\int\ell_{t}^{\ast}(z_{it},a_{it-1})d\boldsymbol{D}_{t}=\int z_{it}d\boldsymbol{D}_{t}=1$
\item \textbf{Goods market:} $Y_{t}=\int c_{t}^{\ast}(z_{it},a_{it-1})d\boldsymbol{D}_{t}+I_{t}$\vspace{2mm}
...
\[
A_{ss}^{hh}\left(\left\{ r_{ss},w_{ss}\right\} \right)=\int a_{ss}^{\ast}(z_{it},a_{it-1},\left\{ r_{ss},w_{ss}\right\} )d\boldsymbol{D}_{ss}
\]
\end{enumerate}
\textbf{Time to code!}
\end{frame}
...
and back out 
\begin{enumerate}
\item $\Gamma_{ss}=Y^{target}/(L_{ss}^{1-\alpha}K_{ss}^{\alpha})$
...
\item Set $L_{ss}=1$ (and $\Pi_{ss}=0$), 
\item Solve infinite horizon household problem \emph{backwards}, i.e. find
$\boldsymbol{a}_{ss}^{\ast}$for a given $\beta$
\item Simulate households \emph{forwards} until convergence, i.e. find $\boldsymbol{D}_{ss}$
\item Return $K_{ss}-\boldsymbol{a}_{ss}^{\ast\prime}\boldsymbol{D}_{ss}$
\item Update $\beta$
\end{enumerate}
...
\begin{align*}
v_{t}(\beta_{i},z_{it},a_{it-1}) & =\max_{c_{it}}\frac{c_{it}^{1-\sigma}}{1-\sigma}+\beta_{i}\mathbb{E}_{t}\left[v_{it+1}(z_{it+1},a_{it})\right]\\
