...
\usepackage{babel}
\begin{document}
	\title{5. Wealth Inequality \vspace{-2mm}}
...
\begin{tabular}{lcccc}
	...
	\hline
	\multicolumn{4}{l}{Wealth (SCF)}  &  \\
	29 & 53 & 80 & 93 & 6 \\
	\hline
	\multicolumn{4}{l}{Earnings (LIS)} &  \\
	6 & 19 & 48 & 72 & 8 \\
	\hline\hline
\end{tabular}
\end{center}
...
%--------------------------------------
\begin{frame}
\frametitle{Saving rate by age and wealth, median earnings level}
...
\item The saving rate of the high wealth households is low or even negative
...
\begin{wideitemize}
\item Counterfactual saving behavior

\item They do not generate the high wealth people that we see in the data

\item They allow for very few saving motives. Might miss important saving motives even for households whose saving behavior we think we understand
...
\subsection{Richer models of wealth inequality}
...
\section{Sources of US Wealth Inequality}
\begin{frame}
...
\item housing  0.037 (incl. imputed rent; Jorda, Knoll, Kuvshinov, Schularick, Tayler 2017)
\end{itemize}
...
Now the authors feed in four different factors that have changed during the past 50 years
\begin{wideitemize}
...
\begin{wideitemize}
\item<1-> the model does a very good job at accounting for the level of wealth inequality
\begin{itemize}
\item (realistic) return heterogeneity is key
\end{itemize}
\item<2-> the model also does a very good job at explaining its evolution over time
\begin{itemize}
...
\item<4-> remaining questions from perspective of this paper:
\begin{itemize}
\item why are portfolios heterogeneous (both across and within wealth levels), what drives returns?
\end{itemize}
\end{wideitemize}
\end{frame}
...
\begin{wideitemize}
	\item <+->\textbf{Today:} Discuss the predictions for wealth inequality arising from Bewley-Huggett-Aiyagari models and their various extensions
	\begin{enumerate}
		\item Difficult to match the fact that wealth more unequal than earnings
		\item Heterogeneity in returns very important
	\end{enumerate}
	\item <+->\textbf{Next week: } Learn to extend the workhorse model to include equilibrium in housing markets
	\item <+->\textbf{Homework:}
	\begin{enumerate}
		%		\item 
		\item Continue to work on previous exercises 
		\item Begin work on the first assignment 
	\end{enumerate}
\end{wideitemize}
